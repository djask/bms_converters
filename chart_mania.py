...
def get_beatmap_data(filepath):
    ...
    metadata['format'] = re.match('osu file format (.+)',f.readline().decode()).group(1)
...
def mania_calc_offset(timings):
    ...
    offset_ms = (first_timing['time'] / first_timing['beatLength'] % 1) * first_timing['beatLength']
...
def bmson_group_mania_soundchannels(hitobjs, timings):
    ...
    for o in hitobjs:
        ...
        sample = o['sample']
        if sample != 'default':
            channel_obj = next(filter(lambda x: x['name'] == sample, sound_channels), None)
        else:
...
def convert_mania_chart(filepath, dstpath, extra_offset):
    ...
    chart_hitobjs = list(map(lambda x: sanitise_mania_hitobj(x, chart_data['CircleSize']),
    ...
    chart_hitobjs = list(map(lambda x: mania_add_offset(x, offset + extra_offset), chart_hitobjs))
    chart_timings = list(map(lambda x: mania_add_offset(x, offset + extra_offset), chart_timings))
...
if __name__ == "__main__":
    ...
    parser.add_option('-p', '--present', dest='preset', default='beatoraja', 
